...
//	}
   ...
   def checkMultipleAnonymousEvents(InterfaceDeclaration element)
   {
      ...
      {
         error("Only 1 anonymous event is allowed per interface",
            anonymousEvents.last, IdlPackage.Literals.EVENT_DECLARATION__NAME
         )
      }
   }
   ...
   def checkSequenceHints(SequenceDeclaration element)
   {
      ...
      if (sizeHints.size > 1)
      {
         error("Typical element size already defined",
            sizeHints.last, IdlPackage.Literals.TYPICAL_SIZE_HINT__SIZE
         )
      }
      ...
      {
         error("Typical sequence length already defined",
            lengthHints.last, IdlPackage.Literals.TYPICAL_LENGTH_HINT__LENGTH
         )
      }
   }
   ...
   def checkModuleStructure(ModuleDeclaration module)
   {
      if (module.isVirtual && module.eContainer instanceof ModuleDeclaration)
      {
         ...
         {
            error("Virtual modules cannot be nested within non-virtual modules",
               module, IdlPackage.Literals.MODULE_DECLARATION__NAME
            )
         }
      }
   }
   ...
   def checkInheritance(InterfaceDeclaration element)
   {
      if (!element.derivesFrom.empty && element.baseTypes.contains(element))
         error(Messages.CIRCULAR_INHERITANCE,
            element, IdlPackage.Literals.INTERFACE_DECLARATION__DERIVES_FROM)
   }
   ...
   def checkInheritance(ExceptionDeclaration element)
   {
      if (element.supertype !== null && element.baseTypes.contains(element))
         error(Messages.CIRCULAR_INHERITANCE,
            element, IdlPackage.Literals.EXCEPTION_DECLARATION__SUPERTYPE)
   }
   ...
   def checkInheritance(StructDeclaration element)
   {
      if (element.supertype !== null && element.baseTypes.contains(element))
         error(Messages.CIRCULAR_INHERITANCE,
            element, IdlPackage.Literals.STRUCT_DECLARATION__SUPERTYPE)
   }
   ...
   def namingCollisions(IDLSpecification idl_specification)
   {
      ...
      for (interface_declaration : idl_specification.eAllContents.filter(InterfaceDeclaration).toIterable)
      {
         if (name_map.containsKey(interface_declaration.name))
            error(Messages.NAME_COLLISION,
               interface_declaration, IdlPackage.Literals.INTERFACE_DECLARATION__NAME)
         else
   ...
   def namingCollisions(StructDeclaration element)
   {
      if (element.supertype !== null)
      {
         ...
         {
            error(Messages.NAME_COLLISION_MEMBERS,
               element, IdlPackage.Literals.STRUCT_DECLARATION__MEMBERS)
         }
      }
   }
   ...
   def checkRelatedEvents(IDLSpecification idl_specification)
   {
      ...
      {
         val related_events = idl_specification.eAllContents.filter(EventDeclaration).filter[data === event_data].toList 
         if (related_events.size > 1)
         {
            for (event : related_events.drop(1))
            {
               error("Event type " + event.data.name + " is already used in another event. Multiple events cannot use the same type due to GUID collisions",
   ...
   def checkTupleTemplateNumber(TupleDeclaration tuple_declaration)
   {
      ...
      {
         error("No more than 8 tuple items (octuple) are supported to comply with the .NET Framework System.Tuple type",
   ...
   def dispatch checkKeywordsAsIdentifiers(EnumDeclaration element)
   {
      ...
      for (identifier : element.containedIdentifiers)
      {
         if (KeywordValidator.isKeyword(identifier, Pattern.LITERAL))
            error(Messages.IDENTIFIER_NAME_IS_KEYWORD, element, IdlPackage.Literals.ENUM_DECLARATION__CONTAINED_IDENTIFIERS)
      }
   }
   ...
   def ensureInterfaceGUID(InterfaceDeclaration element)
   {
      ...
      {
         info("Every interface requires a GUID. Provide a custom one or press CTRL+1 for a Quick Fix. Otherwise it will be added automatically.", IdlPackage.Literals.INTERFACE_DECLARATION__GUID, INTERFACE_GUID)
      }
   }
   ...
   {
      val non_primitive_types =
      element.effectiveMembers
      .filter[ !optional ]
      .filter[ !Util.isSequenceType(type) ]
      .filter[ !Util.isPrimitive(type) ]
   ...
   def checkImmutableOutputParameters(FunctionDeclaration element)
   {
      for (param : element.parameters.filter[direction == ParameterDirection.PARAM_OUT])
      {
         ...
         {
            error(Messages.IMMUTABLE_OUTPUT_PARAMETER, element, IdlPackage.Literals.FUNCTION_DECLARATION__PARAMETERS)
         }
      }
   }
   ...
   def checkDuplicateExceptions(FunctionDeclaration element)
   {
      ...
      {
         warning(Messages.DUPLICATE_EXCEPTION_TYPES, element, IdlPackage.Literals.FUNCTION_DECLARATION__RAISED_EXCEPTIONS)
      }
   }
   ...
   def checkDuplicateExceptions(SequenceDeclaration element)
   {
      if (element.raisedExceptions.size != element.raisedExceptions.toSet.size)
      {
         warning(Messages.DUPLICATE_EXCEPTION_TYPES, element, IdlPackage.Literals.SEQUENCE_DECLARATION__RAISED_EXCEPTIONS)
      }
   }
   ...
   def unsupportedFeatureWarning(FunctionDeclaration element)
   {
      ...
      {
         warning("The 'injectable' keyword has currently no effect on the generated code"
            , element
   ...
   def unsupportedFeatureWarning(SequenceDeclaration element)
   {
      if (element.failable)
      {
         warning("The 'failable' feature is not supported in C#!"
            , element
   ...
   {
      warning("The 'typical sequence length' hint has currently no effect on the generated code"
         , element
   ...
   {
      warning("The 'typical element size' keyword hint currently no effect on the generated code"
         , element
   ...
   def unsupportedFeatureWarning(EventDeclaration element)
   {
      ...
      {
         warning(Messages.FEATURE_NOT_SUPPORED_BY_CAB
            , element
...
//   }
   ...
   {
      error(Messages.EXCEPTION_REFERENCE_NOT_SUPPORTED, element, IdlPackage.Literals.EXCEPTION_REFERENCE_DECLARATION__LOCATION)
   }
   ...
   def unsupportedFeatureError(StructDeclaration element)
   {
      if (element.typeDecls !== null && !element.typeDecls.empty)
         error(Messages.DEPRECATED_NESTED_TYPE_DECLARATION, element, IdlPackage.Literals.STRUCT_DECLARATION__TYPE_DECLS)
