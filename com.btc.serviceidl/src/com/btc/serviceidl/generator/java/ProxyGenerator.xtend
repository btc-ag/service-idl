...
class ProxyGenerator
{
    ...
    def public String generateProxyImplementation(String class_name, InterfaceDeclaration interface_declaration)
    {
        ...
        val serializerType = if (basicJavaSourceGenerator.targetVersion == "0.3") typeResolver.resolve(
        ...
        val deserializerType = if (basicJavaSourceGenerator.targetVersion == "0.3")
                null
            else
        ...
        '''
            public class «class_name» implements «api_name» {
               ...
               .connectService(«api_name».TypeGuid);
                  ...
                  _serializer = 
                    «IF basicJavaSourceGenerator.targetVersion == "0.3"»new «typeResolver.resolve("com.btc.cab.servicecomm.serialization.SinglePartMessageBufferSerializer")»(«ENDIF»
                    new «typeResolver.resolve("com.btc.cab.servicecomm.protobuf.ProtobufSerializer")»()
                    «IF basicJavaSourceGenerator.targetVersion == "0.3"»)«ENDIF»
                    ;
                   «IF deserializerType !== null»
              ...
              .build();
               ...
               «typeResolver.resolve("java.util.concurrent.Future")»<byte[]> requestFuture 
                 = «typeResolver.resolve("com.btc.cab.servicecomm.util.ClientEndpointExtensions")».
                   «IF basicJavaSourceGenerator.targetVersion == "0.3"»RequestAsync«ELSE»requestAsync«ENDIF»
       ...
       */
       ...
       public «typeResolver.resolve(JavaClassNames.CLOSEABLE)» subscribe(«typeResolver.resolve(JavaClassNames.OBSERVER)»<«typeResolver.resolve(anonymous_event.data)»> observer) throws Exception {
          ...
          return «typeResolver.resolve("com.btc.cab.servicecomm.util.EventRegistryExtensions")».subscribe(_endpoint.getEventRegistry()
                .getSubscriberManager(), «IF basicJavaSourceGenerator.targetVersion == "0.3"»_serializerDeserializer«ELSE»_deserializer«ENDIF»,
                «event_type_name».EventTypeGuid,
