...
class InterfaceProjectGenerator extends BasicProjectGenerator
{
    ...
    {
        val paramBundle = ParameterBundle.createBuilder(interfaceDeclaration.moduleStack).with(ProjectType.SERVICE_API).
            build

        // generate all contained types
        // TODO change to Class-based reject with Xtext 2.15
        for (type : interfaceDeclaration.contains.filter(AbstractTypeDeclaration).
            reject[it instanceof AliasDeclaration])
            generateJavaFile(
                projectSourceRootPath.append(Names.plain(type).java),
                paramBundle,
                interfaceDeclaration,
                [new ServiceAPIGenerator(it).generateContainedType(type)]
            )
        ...
        for (event : interfaceDeclaration.namedEvents)
        {
            ...
            generateJavaFile(
                ...
                paramBundle,
                interfaceDeclaration,
                [new ServiceAPIGenerator(it).generateEvent(event)]
            )
        }

        generateJavaFile(projectSourceRootPath.append(
            ProjectType.SERVICE_API.getClassName(ArtifactNature.JAVA, interfaceDeclaration.name).java), paramBundle,
            interfaceDeclaration, [new ServiceAPIGenerator(it).generateMain(interfaceDeclaration)])
        ...
        generateJavaFile(
            projectSourceRootPath.append(serviceFaultHandlerFactoryName.java),
            paramBundle,
            interfaceDeclaration,
            [
                new ServiceFaultHandlerFactoryGenerator(it).
                    generateServiceFaultHandlerFactory(serviceFaultHandlerFactoryName, interfaceDeclaration)
            ]
        )
    }
    ...
    private def void generateTest(IPath projectSourceRootPath, InterfaceDeclaration interfaceDeclaration)
    {
        ...
        generateJavaFile(projectSourceRootPath.append(testName.java), paramBundle, interfaceDeclaration, [
            new TestGenerator(it).generateTestStub(testName, projectSourceRootPath, interfaceDeclaration)
        ])

        val implTestName = interfaceDeclaration.name + "ImplTest"
        generateJavaFile(
            projectSourceRootPath.append(implTestName.java),
            paramBundle,
            interfaceDeclaration,
            [new TestGenerator(it).generateFileImplTest(implTestName, testName, interfaceDeclaration)]
        )

        val zmqTestName = interfaceDeclaration.name + "ZeroMQIntegrationTest"
        generateJavaFile(
            projectSourceRootPath.append(zmqTestName.java),
            paramBundle,
            interfaceDeclaration,
            [
                new TestGenerator(it).generateFileZeroMQItegrationTest(zmqTestName, testName, log4jName,
                    projectSourceRootPath, interfaceDeclaration)
            ]
        )

        if (this.generationSettings.javaTargetVersion == ServiceCommVersion.V0_3)
        {
            fileSystemAccess.generateFile(
                makeProjectSourcePath(interfaceDeclaration, ProjectType.TEST, MavenArtifactType.TEST_RESOURCES,
                    PathType.ROOT).append(log4jName).toPortableString,
    ...
    {
        val paramBundle = ParameterBundle.createBuilder(interfaceDeclaration.moduleStack).with(
            ProjectType.CLIENT_CONSOLE).build
        val programName = "Program"
        val log4jName = "log4j.ClientConsole".properties

        generateJavaFile(
            projectSourceRootPath.append(programName.java),
            paramBundle,
            interfaceDeclaration,
            [new ClientConsoleGenerator(it).generateClientConsoleProgram(programName, log4jName, interfaceDeclaration)]
        )

        if (this.generationSettings.javaTargetVersion == ServiceCommVersion.V0_3)
        {
            fileSystemAccess.generateFile(
                makeProjectSourcePath(interfaceDeclaration, ProjectType.CLIENT_CONSOLE,
                    MavenArtifactType.TEST_RESOURCES, PathType.ROOT).append(log4jName).toPortableString,
                ArtifactNature.JAVA.label,
                ConfigFilesGenerator.generateLog4jProperties()
            )
        }
    ...
    {
        val paramBundle = ParameterBundle.createBuilder(interfaceDeclaration.moduleStack).with(
            ProjectType.SERVER_RUNNER).build
        val programName = "Program"
        ...
        generateJavaFile(
            ...
            interfaceDeclaration,
            [
                new ServerRunnerGenerator(it).generateServerRunnerProgram(programName, serverRunnerName, beansName,
                    log4jName, interfaceDeclaration)
            ]
        )

        generateJavaFile(
            projectSourceRootPath.append(serverRunnerName.java),
            paramBundle,
            interfaceDeclaration,
            [new ServerRunnerGenerator(it).generateServerRunnerImplementation(serverRunnerName, interfaceDeclaration)]
        )

        val packageName = mavenResolver.registerPackage(interfaceDeclaration, ProjectType.SERVER_RUNNER)
        val testResourcesPath = makeProjectSourcePath(interfaceDeclaration, ProjectType.SERVER_RUNNER,
            MavenArtifactType.TEST_RESOURCES, PathType.ROOT)
        fileSystemAccess.generateFile(
    ...
    private def void generateProxy(IPath projectSourceRootPath, InterfaceDeclaration interfaceDeclaration)
    {
        ...
        val proxyFactoryName = ProjectType.PROXY.getClassName(ArtifactNature.JAVA, interfaceDeclaration.name) +
            "Factory"
        generateJavaFile(
            projectSourceRootPath.append(proxyFactoryName.java),
            paramBundle,
            interfaceDeclaration,
            [new ProxyFactoryGenerator(it).generateProxyFactory(proxyFactoryName, interfaceDeclaration)]
        )
        ...
        generateJavaFile(
            ...
            paramBundle,
            interfaceDeclaration,
            [new ProxyGenerator(it).generateProxyImplementation(proxyClassName, interfaceDeclaration)]
        )
    }

    private def void generateDispatcher(IPath projectSourceRootPath, InterfaceDeclaration interfaceDeclaration)
    {
        val paramBundle = ParameterBundle.createBuilder(interfaceDeclaration.moduleStack).with(ProjectType.DISPATCHER).
            build
        val dispatcherClassName = ProjectType.DISPATCHER.getClassName(ArtifactNature.JAVA, interfaceDeclaration.name)

        generateJavaFile(projectSourceRootPath.append(dispatcherClassName.java), paramBundle, interfaceDeclaration, [
            new DispatcherGenerator(it).generateDispatcherBody(dispatcherClassName, interfaceDeclaration)
        ])
    }
    ...
    private def void generateImplementationStub(IPath projectSourceRootPath, InterfaceDeclaration interfaceDeclaration)
    {
        ...
        generateJavaFile(projectSourceRootPath.append(implementationName.java), paramBundle, interfaceDeclaration, [
            new ImplementationStubGenerator(it).generateImplementationStubBody(implementationName, interfaceDeclaration)
        ])
    }
