...
class CppExtensions
{
   ...
   {
      val scope_determinant = if (referenced_object instanceof InterfaceDeclaration) referenced_object else Util.getScopeDeterminant(referenced_object)
      val is_common = (scope_determinant instanceof InterfaceDeclaration)
      
      val file_name = if (is_common) project_type.getClassName(CPP_NATURE, Names.plain(scope_determinant)) else "Types"
      ...
      "modules/" + Util.transform(param_bundle.with(TransformType.FILE_SYSTEM).with(project_type).build) + "/" + include_folder + "/" + file_name + file_extension
   }
    ...
    */
   def private static Collection<TypeWrapper> applyKahnsAlgorithm(HashSet<Pair<EObject, EObject>> graph, List<EObject> independent_elements)
   {
   ...
   {
      val contained_types = element.members
         .filter[Util.getUltimateType(type) instanceof ExceptionDeclaration]
         .map[Util.getUltimateType(type) as ExceptionDeclaration]
