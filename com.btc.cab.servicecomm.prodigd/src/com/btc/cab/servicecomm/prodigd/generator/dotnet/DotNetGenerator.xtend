...
import com.btc.cab.servicecomm.prodigd.idl.AbstractTypeDeclaration
...
class DotNetGenerator
{
   ...
   {
      val impl_class_name = Util.getClassName(param_bundle.build, interface_declaration.name)
   ...
   def private void generateDispatcher(String src_root_path, InterfaceDeclaration interface_declaration)
   {
      ...
      val dispatcher_class_name = Util.getClassName(param_bundle.build, interface_declaration.name)
            ...
            public override «resolve("BTC.CAB.ServiceComm.NET.Common.IMessageBuffer")» ProcessRequest(IMessageBuffer requestBuffer, «resolve("BTC.CAB.ServiceComm.NET.Common.IPeerIdentity")» peerIdentity)
            {
               ...
               «IF func != interface_declaration.functions.head»else «ENDIF»if (request.Has«request_name»)
               {
                  ...
                  «IF !is_void»var result = «ENDIF»_dispatchee.«func.name»
                     (
                        «FOR param : func.parameters SEPARATOR ","»
                           ...
                           «val use_codec = Util.useCodec(param, param_bundle.artifactNature)»
                  ...
                  var responseBuilder = «protobuf_response».Types.«Util.asResponse(func.name)».CreateBuilder()
                     «val use_codec = Util.useCodec(func.returnedType, param_bundle.artifactNature)»
                     ...
                     «FOR param : out_params»
                        ...
                        «val use_codec_param = Util.useCodec(param.paramType, param_bundle.artifactNature)»
   ...
   {
      val codec_name = Util.getCodecName(owner)
   ...
   def private String generateProtobufCodec(EObject owner, String class_name)
   {
      ...
      val data_types = Util.getEncodableTypes(owner)
   ...
   def private String makeEncodeStructOrException(EObject element, Iterable<MemberElementWrapper> members, Optional<Collection<AbstractTypeDeclaration>> type_declarations)
   {
      ...
      «FOR member : members»
         ...
         «val useCodec = Util.useCodec(member.type, param_bundle.artifactNature)»
   ...
   def private String generateLog4NetConfig(ModuleDeclaration module)
   {
      ...
      <log4net>
         <appender name="RollingLogFileAppender" type="log4net.Appender.RollingFileAppender">
            <file value="log_«Util.transform(param_bundle.with(TransformType.PACKAGE).build).toLowerCase».txt"/>
   ...
   def private void generateProxy(String project_root_path, InterfaceDeclaration interface_declaration)
   {
      ...
      val proxy_class_name = Util.getClassName(param_bundle.build, interface_declaration.name)
   ...
   def private String generateProxyFactory(String class_name, InterfaceDeclaration interface_declaration)
   {
      ...
      public class «class_name»
      {
         ...
         {
            return new «Util.getClassName(param_bundle.build, ProjectType.PROXY, interface_declaration.name)»(endpoint);
         }
      }
      '''
   }
         ...
         «FOR function : interface_declaration.functions SEPARATOR System.lineSeparator»
            ...
            )
            {
               ...
               var result =_serviceReference.RequestAsync(new «resolve("BTC.CAB.ServiceComm.NET.Common.MessageBuffer")»(protobufRequest.ToByteArray())).ContinueWith(task =>
               {
                  ...
                  «val use_codec = Util.useCodec(function.returnedType, param_bundle.artifactNature)»
   ...
   def private void generateServiceAPI(String project_root_path, InterfaceDeclaration interface_declaration)
   {
      ...
      reinitializeFile
      file_name = Util.getClassName(param_bundle.build, interface_declaration.name)
      cs_files.add(file_name)
      file_system_access.generateFile(project_root_path + file_name.cs,
      generateSourceFile(
      '''
      «IF !interface_declaration.docComments.empty»
         ...
         /// </summary>
      «ENDIF»
      public interface «Util.getClassName(param_bundle.build, interface_declaration.name)»«IF anonymous_event !== null» : «resolve("System.IObservable")»<«toText(anonymous_event.data, anonymous_event)»>«ENDIF»
      {
   ...
   def private String generateSourceFile(String main_content)
   {
      ...
      «FOR reference : namespace_references.sort AFTER System.lineSeparator»
         using «reference»;
      «ENDFOR»
      namespace «Util.transform(param_bundle.with(TransformType.PACKAGE).build)»
      {
         «main_content»
      }
      '''
      ...
      <Project ToolsVersion="4.0" DefaultTargets="Build" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
        ...
        «IF is_protobuf»
          <PropertyGroup>
            <PreBuildEvent>
            «FOR protobuf_file : protobuf_files»
               protoc.exe --include_imports --proto_path=$(SolutionDir).. --descriptor_set_out=$(ProjectDir)gen/«protobuf_file».protobin $(SolutionDir)../«Util.transform(param_bundle.with(TransformType.FILE_SYSTEM).build)»/gen/«protobuf_file».proto
Protogen.exe -output_directory=$(ProjectDir) $(ProjectDir)gen\«protobuf_file».protobin
   ...
   def private ResolvedName resolve(EObject element, ProjectType project_type)
   {
      ...
      var result = Util.transform(ParameterBundle.createBuilder(Util.getModuleStack(Util.getScopeDeterminant(element))).reset(ArtifactNature.DOTNET).with(project_type).with(TransformType.PACKAGE).build)
   ...
   def private void resolveProjectFilePath(EObject referenced_object, ProjectType project_type)
   {
      ...
      {
         project_path = "../" + Util.getRelativePathsUpwards(param_bundle.build) + Util.transform(temp_param.with(TransformType.FILE_SYSTEM).build) + "/" + project_name
      }
   ...
   {
      return (Util.transform(param_bundle.with(TransformType.PACKAGE).build) == referenced_package.toString)
   }
   ...
   def private String getProtobufRequestClassName(InterfaceDeclaration interface_declaration)
   {
      ...
      return Util.transform(param_bundle.with(TransformType.PACKAGE).with(ProjectType.PROTOBUF).build) + Constants.SEPARATOR_PACKAGE + Util.asRequest(interface_declaration.name)
   }
   ...
   {
      resolve(interface_declaration, ProjectType.PROTOBUF)
      return Util.transform(param_bundle.with(TransformType.PACKAGE).with(ProjectType.PROTOBUF).build) + Constants.SEPARATOR_PACKAGE + Util.asResponse(interface_declaration.name)
   }
   
   def private String getCsprojName(ParameterBundle.Builder builder)
   {
      val project_name = Util.transform(builder.with(TransformType.PACKAGE).build)
   ...
   def private String resolveCodec(EObject object)
   {
      ...
      val codec_name = Util.getCodecName(ultimate_type)
      
      resolveProjectFilePath(ultimate_type, ProjectType.PROTOBUF)
      
      Util.transform(temp_param.with(TransformType.PACKAGE).build) + TransformType.PACKAGE.separator + codec_name
   }
   ...
   {
      Util.transform(param_bundle.with(TransformType.PACKAGE).build).toLowerCase + ".log4net".config
   }
