...
class MavenResolver
{
   ...
   def static MavenDependency resolveDependency(EObject element)
   {
      ...
      return new MavenDependency.Builder()
         .groupId(name)
         .artifactId(name)
         .version(version)
         .build
   ...
   def static Optional<MavenDependency> resolveDependency(String class_name)
   {
      switch name : class_name.toLowerCase
      {
         case name.startsWith("com.google.protobuf."):
            return Optional.of(new MavenDependency.Builder()
               .groupId("com.google.protobuf")
               .artifactId("protobuf-java")
               .version(DependencyVersions.GOOGLE_PROTOBUF)
               .build)
         case name.startsWith("org.apache.commons.collections4."):
            return Optional.of(new MavenDependency.Builder()
               .groupId("org.apache.commons")
               .artifactId("commons-collections4")
               .version(DependencyVersions.COMMONS_COLLECTIONS)
               .build)
         case name.startsWith("org.junit."):
            return Optional.of(new MavenDependency.Builder()
               .groupId("junit")
               .artifactId("junit")
               .version(DependencyVersions.JUNIT)
               .scope("test")
               .build)
        ...
        case name.startsWith("com.btc.cab.servicecomm.protobuf"):
         ...
         case name.startsWith("org.springframework.context.support."):
            return Optional.of(new MavenDependency.Builder()
               ...
               .version(DependencyVersions.SPRING)
               .build)
   ...
   {
      val scope_determinant = Util.getScopeDeterminant(element)
      return Util.transform(ParameterBundle.createBuilder(
         Util.getModuleStack(scope_determinant))
         .reset(ArtifactNature.JAVA).with(TransformType.PACKAGE).build)
