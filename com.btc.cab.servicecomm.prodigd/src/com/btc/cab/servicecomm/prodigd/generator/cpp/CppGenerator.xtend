...
import com.btc.cab.servicecomm.prodigd.idl.AbstractException
...
import static extension com.btc.cab.servicecomm.prodigd.generator.cpp.CppExtensions.*
import com.btc.cab.servicecomm.prodigd.generator.common.ResolvedName
import com.btc.cab.servicecomm.prodigd.generator.common.ProtobufType
import java.util.HashMap
import java.util.Optional
import java.util.UUID
import com.btc.cab.servicecomm.prodigd.idl.ParameterElement
import java.util.Collection
import com.btc.cab.servicecomm.prodigd.generator.common.FeatureProfile
...
import com.btc.cab.servicecomm.prodigd.generator.common.TypeWrapper
...
class CppGenerator
{
   ...
   def private void generateProjectStructure(ProjectType project_type, ModuleDeclaration module)
   {
      ...
      val project_path = param_bundle.artifactNature.label + Constants.SEPARATOR_FILE + Util.transform(param_bundle.with(TransformType.FILE_SYSTEM).build) + Constants.SEPARATOR_FILE
      
      val export_header_file_name = (Util.transform(param_bundle.with(TransformType.EXPORT_HEADER).build) + "_export".h).toLowerCase
   ...
   def private void generateProtobuf(ModuleDeclaration module)
   {
      ...
      // file names
      var export_header_file_name = (Util.transform(param_bundle.with(TransformType.EXPORT_HEADER).build) + "_export".h).toLowerCase
      val dependency_file_name = Constants.FILE_NAME_DEPENDENCIES + ".cpp"
   ...
   def private void generateODB(ModuleDeclaration module)
   {
      ...
      for ( interface_declaration : module.moduleComponents.filter(InterfaceDeclaration))
      {
         val basic_file_name = Util.getClassName(param_bundle.build, interface_declaration.name)
   ...
   def private void generateServerRunner(ModuleDeclaration module)
   {
      ...
      for (interface_declaration : module.moduleComponents.filter(InterfaceDeclaration))
      {
         val cpp_file = Util.getClassName(param_bundle.build, interface_declaration.name).cpp
      ...
      {
         cpp_files.clear
         val project_name = Util.transform(param_bundle.with(TransformType.PACKAGE).build) + TransformType.PACKAGE.separator + interface_declaration.name
         val cpp_file = Util.getClassName(param_bundle.build, interface_declaration.name).cpp
         cpp_files.add(cpp_file)
   ...
   def private void generateProject(ProjectType pt, InterfaceDeclaration interface_declaration, String project_path, String export_header_file_name)
   {
      ...
      // file names
      val main_header_file_name = Util.getClassName(param_bundle.build, interface_declaration.name).h
      val main_cpp_file_name = Util.getClassName(param_bundle.build, interface_declaration.name).cpp
   ...
   {
      val prefix = Util.transform(param_bundle.with(TransformType.EXPORT_HEADER).build).toUpperCase
   ...
   {
      val project_export_macro = Util.transform(param_bundle.with(TransformType.EXPORT_HEADER).build).toUpperCase
   ...
   def private String generateHCodec(EObject owner)
   {
      ...
      val nested_types = Util.getEncodableTypes(owner)
      ...
      val failable_types = Util.getFailableTypes(owner)
      
      val file_content =
      '''
      namespace «Util.getCodecName(owner)»
      {
      ...
      val include_path = "modules"
         ...
         + Util.transform(param_bundle.with(TransformType.FILE_SYSTEM).build)
         + Constants.SEPARATOR_FILE
         + "gen"
         + Constants.SEPARATOR_FILE
         + Util.getPbFileName(owner).pb.h
   ...
   {
      val use_codec = Util.useCodec(element.type, param_bundle.artifactNature)
   ...
   def private String resolveFailableProtobufType(EObject element, EObject container)
   {
      ...
      return namespace + Constants.SEPARATOR_NAMESPACE + Util.asFailable(element, container, qualified_name_provider)
   }
   ...
   def private String generateSource(String file_content, Optional<String> file_tail)
   {
      ...
      «Util.openNamespaces(param_bundle.build)»
         «file_content»
      «Util.closeNamespaces(param_bundle.build)»
   ...
   def private String generateCppProxy(InterfaceDeclaration interface_declaration)
   {
      ...
      «ENDFOR»
      { «getRegisterServerFaults(interface_declaration, Optional.of(Util.transform(param_bundle.with(ProjectType.SERVICE_API).with(TransformType.NAMESPACE).build)))»( GetClientServiceReference().GetServiceFaultHandlerManager() ); }
   ...
   {
      val class_name = Util.getClassName(param_bundle.build, interface_declaration.name)
         ...
         «IF !function.isSync»«resolveCAB("BTC::Commons::CoreExtras::Future")»<«ENDIF»«toText(function.returnedType, interface_declaration)»«IF !function.isSync»>«ENDIF» «class_name.shortName»::«function.name»(«generateParameters(function)»)«IF function.isQuery» const«ENDIF»
         {
            ...
            «IF param_bundle.projectType == ProjectType.PROXY»
               ...
               «FOR param : function.parameters.filter[direction == ParameterDirection.PARAM_IN]»
                  «IF Util.useCodec(param.paramType, param_bundle.artifactNature) && !(Util.isByte(param.paramType) || Util.isInt16(param.paramType) || Util.isChar(param.paramType))»
                     «IF Util.isSequenceType(param.paramType)»
   ...
   def private String generateCppDispatcher(InterfaceDeclaration interface_declaration)
   {
      ...
      { «getRegisterServerFaults(interface_declaration, Optional.of(Util.transform(param_bundle.with(ProjectType.SERVICE_API).with(TransformType.NAMESPACE).build)))»( GetServiceFaultHandlerManager() ); }
      ...
      )
      {
         ...
         if ( request->has_«protobuf_request_method»() )
         {
            ...
            «FOR param : function.parameters.filter[direction == ParameterDirection.PARAM_IN]»
               «IF Util.useCodec(param.paramType, param_bundle.artifactNature)»
                  «IF Util.isSequenceType(param.paramType)»
                     «val ulimate_type = Util.getUltimateType(param.paramType)»
      ...
      void «class_name.shortName»::RegisterMessageTypes(«resolveCAB("BTC::ServiceComm::ProtobufUtil::ProtobufMessageDecoder")» &decoder)
      {
         ...
         decoder.RegisterMessageTypes( 
           ...
           ,buffer
           ,"«Util.switchSeparator(protobuf_request_message.toString, TransformType.NAMESPACE, TransformType.PACKAGE)»"
           ,"«Util.switchSeparator(protobuf_response_message.toString, TransformType.NAMESPACE, TransformType.PACKAGE)»" );
      }
   ...
   def private String makeEncodeResponse(EObject type, EObject container, String protobuf_name, Optional<String> output_param)
   {
      ...
      '''
      «IF Util.useCodec(type, param_bundle.artifactNature) && !(Util.isByte(type) || Util.isInt16(type) || Util.isChar(type))»
   ...
   {
      val use_codec = Util.useCodec(type, param_bundle.artifactNature)
   ...
   def private String generateHClassSignature(InterfaceDeclaration interface_declaration)
   {
      ...
      '''«Util.getClassName(param_bundle.build, interface_declaration.name)» : 
      «IF is_api»
   ...
   {
      return Util.transform(param_bundle.with(TransformType.EXPORT_HEADER).build).toUpperCase
   ...
   def private ResolvedName resolve(EObject object, ProjectType project_type)
   {
      ...
      if (HeaderResolver.isCAB(resolved_name))
         resolveCAB(Util.switchPackageSeperator(resolved_name, TransformType.NAMESPACE))
      else if (HeaderResolver.isBoost(resolved_name))
         resolveBoost(Util.switchPackageSeperator(resolved_name, TransformType.NAMESPACE))
      else
      {
         var result = Util.transform(ParameterBundle.createBuilder(Util.getModuleStack(Util.getScopeDeterminant(object))).with(project_type).with(TransformType.NAMESPACE).build)
   ...
   def private ResolvedName resolveProtobuf(EObject object, ProtobufType protobuf_type)
   {
      ...
      var result = Util.transform(builder.with(TransformType.NAMESPACE).build)
      ...
      var header_path = Util.transform(builder.with(TransformType.FILE_SYSTEM).build)
      var header_file = Util.getPbFileName(object)
   ...
   def private void resolveProjectFilePath(EObject referenced_object, ProjectType project_type)
   {
      ...
      val project_path = '''$(SolutionDir)\«Util.transform(temp_param.with(TransformType.FILE_SYSTEM).build).replace(Constants.SEPARATOR_FILE, Constants.SEPARATOR_BACKSLASH)»\«project_name»'''
   ...
   def private String resolveCodecNS(EObject object, boolean is_failable, Optional<EObject> container)
   {
      ...
      val codec_name = if (is_failable) Util.getCodecName(container.get) else Util.getCodecName(ultimate_type)
      
      var header_path = Util.transform(temp_param.with(TransformType.FILE_SYSTEM).build)
      modules_includes.add("modules/" + header_path + "/include/" + codec_name.h)
      resolveProjectFilePath(ultimate_type, ProjectType.PROTOBUF)
      
      Util.transform(temp_param.with(TransformType.NAMESPACE).build) + TransformType.NAMESPACE.separator + codec_name
   }
   
   def private String getVcxprojName(ParameterBundle.Builder builder, Optional<String> extra_name)
   {
      var project_name = Util.transform(builder.with(TransformType.PACKAGE).build)
   ...
   def private boolean isMutableField(EObject type)
   {
      ...
      val use_codec = Util.useCodec(ultimate_type, param_bundle.artifactNature)
